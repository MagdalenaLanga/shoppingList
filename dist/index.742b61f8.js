...
//module Input Values
class InputValues {
    constructor(name, ingridientName, amount, unit1){
        this.name = name;
...
const addIngBtnOnClick = ()=>{
    ...
    } else alert('Wpisz nazwę przepisu');
...
const addCheckboxes = ()=>{
    ...
    allIngridients.forEach((ing)=>{
        if ((ing.ingridientName && ing.amount && ing.unit) !== "") whole.insertAdjacentHTML("beforeEnd", `<div class="item">
                    <input type="checkbox">
                    <span>${ing.ingridientName} ${ing.amount} ${ing.unit}</span>
                </div>`);
        else alert("Wpisz składnik");
    });
    hideIngridients();
};
...
const collectValues = ()=>{
    ...
    for(i = 0; i < inputsChildren.length; i++)if (recipeName.value && inputsChildren[i][0].value && inputsChildren[i][1].value && inputsChildren[i][0].value) {
...
//# sourceMappingURL=index.742b61f8.js.map
